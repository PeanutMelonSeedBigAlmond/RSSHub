...
module.exports = async (ctx) => {
    ...
    for (let i = 0; i < illusts.length; i++) {
        if (illusts[i].type === 'ugoira') {
            const data = await handleUgoira(illusts[i].id, token);
            illusts[i].zipUrl = data.zipUrl;
            illusts[i].frames = data.frames;
        }
    }
    ...
    ctx.state.data = {
        ...
        item: illusts.map(async (illust) => {
            ...
            if (illust.type === 'ugoira') {
                ...
                const imagesProceed = images.map((it) => {
                    return `<img src=${it}/></br>`;
                });
